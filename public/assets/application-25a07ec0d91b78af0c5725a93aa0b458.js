...
(function ($) {
  console.log("mobileinit.js")
    ...
    $(document).on("pageinit", "#talks-control", function(event0, data0){
      ...
      talks_control_init(event0, data0);
      // $('#talk_talk_date').html("hi");
    })
    console.log("trap on talks_control_init set")
    */
...
/* 
  ...
  -- code here will be executed, but in its own private namespace, on initial setup of the system
  -- starting with talks.js to avoid syntax questions with coffeescript
*/


$(document).on("pageinit", "#talks-control", function(event0, data0) {
  console.log("We (should) have just initialized the talks-control page");  // and so we have!
  try {
    $('#talk_talk_date').datepicker();
    console.log("have attached datepicker to talk_date");
  } catch(exception) {
    console.error("not able to attach datepicker");
