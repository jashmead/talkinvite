...
1. placeholder instead of labels -- actually best to do both, see form_for_jqm
...
1. verify that all forms have been given a valid @record before they get started; log errors if they haven't been -- this is spotted during all but the most primitive testing
1. kill all offset, span references -- appears done

=== TBD

1. email form for contact, for now, just an email href
...
1. check for icons for each page:  arrows, delete, plus, gear, refresh, grid, star, alert, info, home, search and they can be left, right, and so on, as well as notext -- do in detail, -- done
...
1. switch updates to reverts (disabled until a change made or make invisible)
1. add duplicate/clone button to those forms that could benefit from it
1. add tooltips to everything
1. add all help pages
