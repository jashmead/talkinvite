...
--
Dev notes:
  Nested includes don't currently work, at least not reliably, in rdoc, so include sub files by hand
  This documentation is really the work-plan
++
...
--
Most important bit, after the objective, is the stories
++
...
* Use bootstrap & liquid design, top to bottom, left to right
...
== Stages
1. people -- started, search now up, should recheck usual CRUD & the account pages (signin, settings, & so on)
1. talks -- started, current hot spot is 'start'
1. relationships -- started as follower/followed, monitor as work progresses
1. posts -- next hot spot, once 'start' is working
1. notifications -- generic layer, test with posts? rss?
